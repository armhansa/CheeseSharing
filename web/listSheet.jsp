...
<sql:query var="filter_faculty" dataSource="${dataSource}">
    SELECT * FROM SHEETS WHERE Faculty = '<%= request.getParameter("filter") %>';
</sql:query>

<sql:query var="filter_category" dataSource="${dataSource}">
...
<body style="font-family: 'Kanit'">
    ...
    <div class="list-sheet">
        ...
        <!-- แสดง lsit file ของคณะที่เลือกมา -->
        <c:forEach var="list" items="${filter_faculty.rows}">
        <div class="sheet">
            <div class="detail">
    ...
    <% } else { %>
    <!-- แสดง lsit file ของวิชาที่เลือกมา -->
    <c:forEach var="list" items="${filter_category.rows}">
    <div class="sheet">
        <div class="detail">
            <h3 style="margin-left: 10px">${list.Title}</h3>
            <p style="margin-left: 10px">ลงโดย : ${list.USERS_Username}</p>
